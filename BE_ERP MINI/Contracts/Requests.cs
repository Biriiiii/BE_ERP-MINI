...
public sealed record CreateSalesInvoiceRequest(string CustomerName, string? CustomerPhone, int? CustomerId, int CreatedBy, string? Notes, int PaymentStatus, int PaymentMethod, decimal? PaidAmount, List<CreateSalesInvoiceLineRequest> Lines);
...
public sealed record SwitchPaymentRequest(int PaymentMethod, int PaymentStatus, decimal? PaidAmount = null);
